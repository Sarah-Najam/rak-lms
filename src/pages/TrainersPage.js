...
function TrainersPage() {
  ...
  const emptyForm = { name: '', institute: '', rating: 0, phone: '', email: '', expertise: '', bio: '', type: 'Internal' };
  ...
  const Stars = ({ value, size = 14 }) => (
    <span>
      {[1,2,3,4,5].map(i => (
        <span key={i} style={{ color: i <= Math.round(value) ? '#c8973a' : '#d4d9dd', fontSize: size + 'px' }}>★</span>
      ))}
      {value > 0 && <span style={{ fontSize: '11px', color: '#9baabb', marginLeft: '3px' }}>{value}</span>}
    </span>
  );
  ...
  const StarRater = ({ value, onChange }) => (
    <div style={{ display: 'flex', gap: '4px' }}>
      {[1,2,3,4,5].map(star => (
        <button key={star} style={{ background: 'none', border: 'none', cursor: 'pointer', padding: '2px', fontSize: '24px', color: star <= (hoverRating || value) ? '#c8973a' : '#d4d9dd' }}
  ...
  return (
    <div style={styles.page}>
      ...
      <div style={styles.controls}>
        ...
        <button style={styles.addBtn} onClick={() => setShowAdd(true)}>Add Trainer</button>
      </div>

      {loading ? (
        <div style={{ padding: '40px', textAlign: 'center', color: '#9baabb' }}>Loading trainers...</div>
      ) : (
            ...
            <tbody>
              {filtered.map((trainer, i) => (
                <tr key={trainer.id} style={styles.tr}>
                  ...
                  <td style={styles.td}>
                    <button style={styles.nameBtn} onClick={() => setSelected(trainer)}>{trainer.name}</button>
                  </td>
                  <td style={{ ...styles.td, color: '#5a6878', fontSize: '12.5px' }}>{trainer.institute || '—'}</td>
                  <td style={{ ...styles.td, fontSize: '12px', color: '#5a6878' }}>
                    {Array.isArray(trainer.expertise) ? trainer.expertise.slice(0,2).join(', ') : trainer.expertise || '—'}
                  </td>
                  <td style={styles.td}>
                    <span style={{ ...styles.typeBadge, background: trainer.type === 'Internal' ? '#f0f9ff' : '#fdf4ff', color: trainer.type === 'Internal' ? '#0369a1' : '#7c3aed' }}>
    ...
    🗑️
          ...
          {filtered.length === 0 && (
            <div style={{ padding: '40px', textAlign: 'center', color: '#9baabb', fontSize: '14px' }}>
              {trainers.length === 0 ? 'No trainers yet. Click "Add Trainer" to get started.' : 'No trainers found.'}
            </div>
          )}
        </div>
      )}
          ...
          <div style={styles.modal} onClick={e => e.stopPropagation()}>
            ...
            <div style={styles.modalBody}>
              <div style={{ display: 'flex', gap: '16px', alignItems: 'center', marginBottom: '16px' }}>
                ...
                <div style={{ flex: 1 }}>
                  <F label="Trainer Name *" value={form.name} onChange={v => setForm({...form, name: v})} placeholder="Full name" />
                </div>
              </div>
              ...
              <div style={styles.formGrid}>
                ...
                <F label="Area of Expertise (comma separated)" value={form.expertise} onChange={v => setForm({...form, expertise: v})} placeholder="e.g. Leadership, Sales" />
              </div>
              <div style={{ marginTop: '4px' }}>
                <label style={styles.fieldLabel}>Bio</label>
                <textarea value={form.bio} onChange={e => setForm({...form, bio: e.target.value})}
                  placeholder="Brief background of the trainer"
                  style={{ width: '100%', padding: '10px 12px', border: '1.5px solid #e8ecf0', borderRadius: '8px', fontSize: '13px', outline: 'none', background: '#f8f9fa', resize: 'vertical', minHeight: '70px', fontFamily: 'Inter, sans-serif' }} />
              </div>
            </div>
      ...
      {selected && (
        <div style={styles.overlay} onClick={() => setSelected(null)}>
          <div style={{ ...styles.modal, maxWidth: '600px' }} onClick={e => e.stopPropagation()}>
            ...
            <div style={styles.modalBody}>
              <div style={{ display: 'flex', alignItems: 'center', gap: '16px', padding: '16px 18px', background: '#f8f9fa', borderRadius: '10px', marginBottom: '18px', border: '1px solid #e8ecf0' }}>
                <div style={{ width: '54px', height: '54px', borderRadius: '50%', background: '#051c2c', color: '#ffffff', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '18px', fontWeight: '700', flexShrink: 0 }}>
                  {initials(selected.name)}
                </div>
                <div style={{ flex: 1 }}>
                  <div style={{ fontSize: '16px', fontWeight: '700', color: '#051c2c' }}>{selected.name}</div>
                  <div style={{ fontSize: '12px', color: '#5a6878', marginTop: '3px' }}>{selected.institute}</div>
                  <div style={{ marginTop: '6px' }}><Stars value={selected.rating} size={13} /></div>
                </div>
                <span style={{ ...styles.typeBadge, background: selected.type === 'Internal' ? '#f0f9ff' : '#fdf4ff', color: selected.type === 'Internal' ? '#0369a1' : '#7c3aed', alignSelf: 'flex-start' }}>
                  {selected.type || 'External'}
                </span>
              </div>
              {selected.bio && (
                <div style={{ fontSize: '13px', color: '#5a6878', lineHeight: 1.65, padding: '13px 16px', background: '#f8f9fa', borderRadius: '8px', borderLeft: '3px solid #051c2c', marginBottom: '16px' }}>
                  {selected.bio}
                </div>
              )}
              {selected.expertise && selected.expertise.length > 0 && (
                <div style={{ marginBottom: '16px' }}>
                  <div style={{ fontSize: '11px', fontWeight: '700', color: '#5a6878', textTransform: 'uppercase', letterSpacing: '0.5px', marginBottom: '8px' }}>Areas of Expertise</div>
                  <div style={{ display: 'flex', flexWrap: 'wrap', gap: '6px' }}>
                    {(Array.isArray(selected.expertise) ? selected.expertise : [selected.expertise]).map((e, i) => (
                      <span key={i} style={{ background: '#f2f4f6', border: '1px solid #e8ecf0', borderRadius: '20px', padding: '3px 10px', fontSize: '12px', fontWeight: '500', color: '#051c2c' }}>{e}</span>
                    ))}
                  </div>
                </div>
              )}
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '14px', marginBottom: '16px' }}>
                <div>
                  ...
                  <div style={{ fontSize: '13px', fontWeight: '500', color: '#051c2c', marginTop: '4px' }}>{selected.phone || '—'}</div>
                </div>
                <div>
                  <div style={styles.fieldLabel}>Email</div>
                  <div style={{ fontSize: '13px', fontWeight: '500', color: '#051c2c', marginTop: '4px' }}>{selected.email || '—'}</div>
                </div>
              </div>
              <div style={{ display: 'flex', gap: '8px' }}>
                <button style={styles.cancelBtn}>📎 Upload CV</button>
                <button style={styles.cancelBtn}>⬇ Download CV</button>
              </div>
            </div>
            <div style={styles.modalFooter}>
...
function F({ label, value, onChange, placeholder, type = 'text', options = [] }) {
  const inputStyle = { width: '100%', padding: '10px 12px', border: '1.5px solid #e8ecf0', borderRadius: '8px', fontSize: '13px', outline: 'none', background: '#f8f9fa', color: '#051c2c', fontFamily: 'Inter, sans-serif' };
  return (
...
const styles = {
  ...
  nameBtn:    { background: 'none', border: 'none', cursor: 'pointer', fontSize: '13px', fontWeight: '600', color: '#051c2c', padding: 0, fontFamily: 'Inter, sans-serif' },
