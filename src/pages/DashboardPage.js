...
function DashboardPage() {
  ...
  const deptColor = dept => {
    const map = { Operations: { bg: '#dbeafe', text: '#1d4ed8' }, HR: { bg: '#dcfce7', text: '#15803d' }, Sales: { bg: '#fef9c3', text: '#a16207' }, Finance: { bg: '#f3e8ff', text: '#7c3aed' } };
      ...
      <div style={styles.mainGrid}>
        <div style={styles.leftCol}>
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            <span style={{ fontSize: '18px', fontWeight: '700', color: '#051c2c' }}>Training Summary</span>
          </div>
          ...
          <div style={styles.chartsRow}>
            <div style={styles.chartCard}>
              <div style={styles.chartHeader}>
                ...
                <div style={styles.toggleGroup}>
                  {['Monthly','Quarterly'].map(v => (
                    <button key={v} onClick={() => setSatView(v)} style={{ ...styles.toggleBtn, ...(satView === v ? styles.toggleBtnActive : {}) }}>{v}</button>
                  ))}
                </div>
              </div>
              <ResponsiveContainer width="100%" height={160}>
                <LineChart data={satisfactionTrendData}>
                  ...
                  <YAxis domain={[3.5, 4.5]} tick={{ fontSize: 11, fill: '#9baabb' }} axisLine={false} tickLine={false} />
                  <Tooltip contentStyle={{ fontSize: '12px', borderRadius: '8px', border: '1px solid #e8ecf0' }} />
                  <Line type="monotone" dataKey="score" stroke="#051c2c" strokeWidth={2} dot={{ fill: '#051c2c', r: 4 }} activeDot={{ r: 6 }} />
                </LineChart>
              </ResponsiveContainer>
              <div style={{ fontSize: '11px', color: '#9baabb', textAlign: 'center', marginTop: '6px' }}>Satisfaction Trend</div>
            </div>
            ...
            <div style={styles.chartCard}>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', padding: '10px 0', gap: '8px' }}>
                <div style={{ width: '110px', height: '110px', borderRadius: '50%', background: 'conic-gradient(#051c2c 0% 84%, #e8ecf0 84% 100%)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                  <div style={{ width: '82px', height: '82px', borderRadius: '50%', background: '#ffffff', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center' }}>
                    <span style={{ fontSize: '22px', fontWeight: '800', color: '#051c2c', lineHeight: 1 }}>{avgSatisfaction}</span>
                    <span style={{ fontSize: '14px', color: '#c8973a', marginTop: '2px' }}>★</span>
                  </div>
                </div>
                <div style={{ fontSize: '12px', fontWeight: '600', color: '#051c2c', textAlign: 'center' }}>Overall Satisfaction Score</div>
                <div style={{ fontSize: '11px', color: '#9baabb', textAlign: 'center' }}>Based on {depts.length * 68} feedbacks</div>
              </div>
            </div>
          </div>
        </div>
        ...
        <div style={styles.rightCol}>
          <div style={styles.calendarCard}>
            <div style={{ fontSize: '14px', fontWeight: '700', color: '#ffffff', marginBottom: '14px' }}>Upcoming Training Calendar</div>
            <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '12px' }}>
              <thead>
                <tr>
                  {['Date','Course Title','Trainer','Department','Enrolled'].map(h => (
                    <th key={h} style={{ padding: '6px 8px', textAlign: 'left', color: 'rgba(182,189,194,0.6)', fontWeight: '600', fontSize: '10px', textTransform: 'uppercase', letterSpacing: '0.4px', borderBottom: '1px solid rgba(255,255,255,0.08)' }}>{h}</th>
                  ))}
                </tr>
              </thead>
              <tbody>
                {upcomingTraining.map((row, i) => (
                  <tr key={i} style={{ background: i % 2 === 0 ? 'rgba(255,255,255,0.03)' : 'transparent' }}>
                    <td style={{ padding: '9px 8px', color: 'rgba(255,255,255,0.85)', borderBottom: '1px solid rgba(255,255,255,0.05)', fontSize: '12px' }}>{row.date}</td>
                    <td style={{ padding: '9px 8px', color: 'rgba(255,255,255,0.85)', borderBottom: '1px solid rgba(255,255,255,0.05)', fontSize: '12px' }}>{row.title}</td>
                    <td style={{ padding: '9px 8px', color: 'rgba(255,255,255,0.85)', borderBottom: '1px solid rgba(255,255,255,0.05)', fontSize: '12px' }}>{row.trainer}</td>
                    <td style={{ padding: '9px 8px', borderBottom: '1px solid rgba(255,255,255,0.05)' }}>
                      <span style={{ background: deptColor(row.dept).bg, color: deptColor(row.dept).text, padding: '2px 8px', borderRadius: '12px', fontSize: '10px', fontWeight: '600' }}>{row.dept}</span>
                    </td>
                    <td style={{ padding: '9px 8px', color: 'rgba(255,255,255,0.85)', borderBottom: '1px solid rgba(255,255,255,0.05)', fontSize: '12px', fontWeight: 600 }}>{row.enrolled}</td>
                  </tr>
                ))}
              </tbody>
            </table>
          ...
          <div style={styles.chartCard}>
            <div style={styles.chartHeader}>
              ...
              <div style={styles.toggleGroup}>
                {['Monthly','Quarterly'].map(v => (
                  <button key={v} onClick={() => setCalView(v)} style={{ ...styles.toggleBtn, ...(calView === v ? styles.toggleBtnActive : {}) }}>{v}</button>
                ))}
              </div>
            </div>
            <ResponsiveContainer width="100%" height={160}>
