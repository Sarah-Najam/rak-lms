...
function CoursesPage({ user }) {
  ...
  useEffect(() => {
    if (!filterDeptId) { setDeptCourseIds(null); return; }
  ...
  const filtered = courses.filter(c => {
    ...
    if (filterStartDate && courseDate) matchDateRange = matchDateRange && new Date(courseDate) >= new Date(filterStartDate);
    if (filterEndDate && courseDate)   matchDateRange = matchDateRange && new Date(courseDate) <= new Date(filterEndDate);
  ...
  const openTrainerPopup = (trainerName) => {
    ...
    const trainer = trainers.find(t => t.name.toLowerCase() === trainerName.toLowerCase());
  ...
  const handleToggleManual = async (isManual) => {
    const result = await api.toggleManualSatisfaction(feedbackPopup.id, isManual, isManual ? manualValue : null);
    if (result.id) { setManualOverride(isManual); loadCourses(); setFeedbackPopup(result); }
  };

  const handleSaveManualValue = async () => {
    const result = await api.toggleManualSatisfaction(feedbackPopup.id, true, manualValue);
    if (result.id) { loadCourses(); setFeedbackPopup(result); alert('Satisfaction rate updated.'); }
  };
  ...
  const handleSave = async () => {
    ...
    try {
      const newCourse = await api.addCourse({
        title: form.title, description: form.description, institute: form.institute,
        trainer_name: form.trainer, venue: form.venue,
        start_date: form.startDate || null, end_date: form.endDate || null,
        duration_hours: +form.hours || 0, duration_days: +form.duration || 0,
        cost_estimated: +form.cost || 0, budget_realized: +form.budgetRealized || 0,
        type: form.type, status: form.status, max_learners: +form.maxLearners || null,
        po_number: form.po, pr_number: form.pr, table_of_contents: form.tableOfContents,
        stars: +form.stars || 0, training_type: form.trainingType,
      });
      if (newCourse.id) { loadCourses(); setForm(emptyForm); setShowAdd(false); }
      else alert(newCourse.error || 'Could not save course.');
  ...
  const handleEditSave = async () => {
    ...
    try {
      const updated = await api.updateCourse(editCourse.id, {
        title: editForm.title, description: editForm.description, institute: editForm.institute,
        trainer_name: editForm.trainer, venue: editForm.venue,
        start_date: editForm.startDate || null, end_date: editForm.endDate || null,
        duration_hours: +editForm.hours || 0, duration_days: +editForm.duration || 0,
        cost_estimated: +editForm.cost || 0, budget_realized: +editForm.budgetRealized || 0,
        type: editForm.type, status: editForm.status, max_learners: +editForm.maxLearners || null,
        po_number: editForm.po, pr_number: editForm.pr, table_of_contents: editForm.tableOfContents,
        stars: +editForm.stars || 0, training_type: editForm.trainingType,
      });
      if (updated.id) { loadCourses(); setEditCourse(null); }
      else alert(updated.error || 'Could not update course.');
    } catch (err) { alert('Error connecting to server.'); }
  };
  ...
  const openEdit = (course) => {
    setEditForm({
      ...
      duration: course.duration_days || '', institute: course.institute || '',
      hours: course.duration_hours || '', cost: course.cost_estimated || '',
      ...
      type: course.type || 'External', status: course.status || 'Pending',
      tableOfContents: course.table_of_contents || '', maxLearners: course.max_learners || '',
      trainer: course.trainer_name || '', po: course.po_number || '',
      pr: course.pr_number || '', venue: course.venue || '',
      stars: course.stars || 0, trainingType: course.training_type || 'Developmental',
    });
  ...
  const handleDelete = async (id) => {
    ...
    try { await api.deleteCourse(id); loadCourses(); }
  ...
  const enrolledDepts = [...new Set(enrolledLearners.map(l => l.department_name).filter(Boolean))];
  const filteredEnrolled = deptFilter ? enrolledLearners.filter(l => l.department_name === deptFilter) : enrolledLearners;
  const deptBreakdown = enrolledDepts.map(dept => {
    const deptLearners = enrolledLearners.filter(l => l.department_name === dept);
    return { dept, total: deptLearners.length, attended: deptLearners.filter(l => l.attended).length };
  ...
  const Stars = ({ value, showPct = false }) => {
    ...
    return (
      <span>
        ...
        {+value > 0 && <span style={{ fontSize: '11px', color: '#9baabb', marginLeft: '3px' }}>{value}/5</span>}
      </span>
    );
  };
    ...
    <div key={course.id} style={styles.upcomingCard}>
      ...
      <div style={styles.upcomingCardMeta}>
        <div style={styles.upcomingMetaItem}>
          ...
          <button style={{ background: 'none', border: 'none', padding: 0, fontSize: '12px', color: '#0369a1', cursor: 'pointer', fontFamily: 'Inter, sans-serif', textDecoration: 'underline' }}
            onClick={() => openTrainerPopup(course.trainer_name)}>
            {course.trainer_name || '—'}
          </button>
        </div>
        <div style={styles.upcomingMetaItem}><span style={styles.upcomingMetaIcon}>📅</span><span>{fmtDate(course.start_date)}</span></div>
        <div style={styles.upcomingMetaItem}><span style={styles.upcomingMetaIcon}>📍</span><span>{course.venue || '—'}</span></div>
        <div style={styles.upcomingMetaItem}><span style={styles.upcomingMetaIcon}>👥</span><span>{course.enrolled_count || 0} enrolled</span></div>
      </div>
      <button style={styles.upcomingViewBtn} onClick={() => openDetail(course)}>View Details →</button>
    </div>
  );
        ...
        <div style={styles.upcomingSection}>
          ...
          {!devCollapsed && (
            upcomingDevelopmental.length > 0
              ? <div style={styles.upcomingGridSingle}>{upcomingDevelopmental.map(renderUpcomingCard)}</div>
        ...
        <div style={{ ...styles.upcomingSection, borderColor: '#bfdbfe' }}>
          ...
          {!mandCollapsed && (
            upcomingMandatory.length > 0
              ? <div style={styles.upcomingGridSingle}>{upcomingMandatory.map(renderUpcomingCard)}</div>
      ...
      <div style={styles.controls}>
        <div style={{ display: 'flex', gap: '10px', alignItems: 'center', flexWrap: 'wrap' }}>
          <div style={styles.searchWrap}>
            <span>🔍</span>
            <input style={styles.searchInput} placeholder="Search by Course Title" value={searchTitle}
              onChange={e => { setSearchTitle(e.target.value); setCurrentPage(1); }} />
          </div>
          {!isHod && (
            <select value={filterDeptId} onChange={e => { setFilterDeptId(e.target.value); setCurrentPage(1); }} style={styles.filterSelect}>
...
<span style={{ fontSize: '11px', fontWeight: '400', marginLeft: '10px', display: 'inline-flex', alignItems: 'center', gap: '10px' }}>
  ...
  <span style={{ color: '#5a6878' }}>Mandatory</span>
            ...
            <table style={{ ...styles.table, minWidth: '1080px' }}>
              ...
              <tbody>
                {paginated.map((course, i) => {
                  ...
                  const totalHours = (+course.duration_hours || 0) * attended;
                  ...
                  return (
                    <tr key={course.id} style={styles.tr}>
                      ...
                      <td style={{ ...styles.td, minWidth: '130px' }}>
                        {course.trainer_name
                          ? <button style={styles.trainerNameBtn} onClick={() => openTrainerPopup(course.trainer_name)}>{course.trainer_name}</button>
                          : '—'}
                      </td>
                      <td style={{ ...styles.td, fontSize: '12px', color: '#5a6878', whiteSpace: 'nowrap' }}>{fmtDate(course.start_date)}</td>
                      <td style={{ ...styles.td, fontSize: '12px', color: '#5a6878', whiteSpace: 'nowrap' }}>{fmtDate(course.end_date)}</td>
                      <td style={styles.td}>
                      ...
                      <td style={styles.td}>
                        <span style={{
                          background: attended === 0 ? '#f1f5f9' : attended / Math.max(enrolled, 1) >= 0.9 ? '#dcfce7' : attended / Math.max(enrolled, 1) >= 0.7 ? '#fef9c3' : '#fee2e2',
                          color: attended === 0 ? '#475569' : attended / Math.max(enrolled, 1) >= 0.9 ? '#15803d' : attended / Math.max(enrolled, 1) >= 0.7 ? '#a16207' : '#991b1b',
                          padding: '2px 8px', borderRadius: '10px', fontSize: '11px', fontWeight: '700',
                        }}>{attRate}</span>
                      </td>
                      ...
                      {!isHod && (
                        <td style={{ ...styles.td, whiteSpace: 'nowrap' }}>
                          <div style={{ display: 'flex', gap: '6px' }}>
                            <button style={styles.actionBtn} onClick={() => openEdit(course)} title="Edit">✏️</button>
                            <button style={{ ...styles.actionBtn, background: '#fee2e2' }} onClick={() => handleDelete(course.id)} title="Delete">🗑️</button>
                          </div>
                        </td>
                      )}
                    </tr>
          ...
          <Pagination currentPage={currentPage} totalItems={totalFiltered} itemsPerPage={ITEMS_PER_PAGE} onPageChange={setCurrentPage} />
        </div>
      )}
      ...
      {showAdd && !isHod && (
        <div style={styles.overlay} onClick={() => setShowAdd(false)}>
          <div style={styles.modal} onClick={e => e.stopPropagation()}>
            ...
            <div style={styles.modalBody}><CourseForm f={form} setF={setForm} trainers={trainers} /></div>
      ...
      {editCourse && !isHod && (
        <div style={styles.overlay} onClick={() => setEditCourse(null)}>
          <div style={styles.modal} onClick={e => e.stopPropagation()}>
            ...
            <div style={styles.modalBody}><CourseForm f={editForm} setF={setEditForm} trainers={trainers} /></div>
            <div style={styles.modalFooter}>
      ...
      {/* ── COURSE DETAIL POPUP ── */}
      {selected && (
        <div style={styles.overlay} onClick={() => { setSelected(null); setProfileLearner(null); setDeptFilter(''); }}>
...
<div style={styles.coverImg}>
              ...
              <div style={styles.infoBar}>
                {[
                  ...
                  <div key={k} style={styles.infoBarItem}>
                    ...
                    <div style={{ ...styles.infoBarValue, color: k === 'Attended' ? '#15803d' : '#051c2c' }}>{v}</div>
                  </div>
                ))}
              </div>
              ...
              {(+selected.enrolled_count || 0) > 0 && (
                <div style={{ marginBottom: '20px' }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '6px' }}>
                    <span style={{ fontSize: '12px', fontWeight: '600', color: '#051c2c' }}>Enrolled vs Attended</span>
                    <span style={{ fontSize: '12px', color: '#5a6878' }}>{selected.attended_count || 0} of {selected.enrolled_count} attended</span>
                  </div>
                  <div style={{ height: '8px', background: '#e8ecf0', borderRadius: '4px', overflow: 'hidden' }}>
                    <div style={{ height: '100%', width: Math.round((+selected.attended_count || 0) / (+selected.enrolled_count) * 100) + '%', background: '#051c2c', borderRadius: '4px' }} />
                  </div>
                </div>
              )}
              ...
              <div style={styles.detailGrid}>
                ...
                <div>
                  <div style={{ fontSize: '10px', fontWeight: '700', color: '#9baabb', textTransform: 'uppercase', letterSpacing: '0.5px', marginBottom: '4px' }}>Trainer</div>
                  {selected.trainer_name
                    ? <button style={{ background: 'none', border: 'none', padding: 0, cursor: 'pointer', fontSize: '13px', fontWeight: '600', color: '#0369a1', fontFamily: 'Inter, sans-serif', textDecoration: 'underline' }} onClick={() => openTrainerPopup(selected.trainer_name)}>{selected.trainer_name}</button>
                    : '—'}
                </div>
                <div>
                  <div style={{ fontSize: '10px', fontWeight: '700', color: '#9baabb', textTransform: 'uppercase', letterSpacing: '0.5px', marginBottom: '4px' }}>Satisfaction Rate</div>
                  <button style={{ background: 'none', border: 'none', cursor: 'pointer', padding: 0 }} onClick={() => openFeedbackPopup(selected)}>
                    {+selected.stars > 0
              ...
              {!enrollLoading && deptBreakdown.length > 0 && (
                <div style={{ marginTop: '20px' }}>
                  ...
                  <div style={{ display: 'flex', flexWrap: 'wrap', gap: '8px', marginBottom: '14px' }}>
                    <button onClick={() => setDeptFilter('')} style={{ padding: '5px 12px', borderRadius: '20px', border: 'none', fontSize: '12px', fontWeight: '600', cursor: 'pointer', fontFamily: 'Inter, sans-serif', background: deptFilter === '' ? '#051c2c' : '#f2f4f6', color: deptFilter === '' ? '#ffffff' : '#5a6878' }}>
                      All ({enrolledLearners.length})
                    </button>
                    {deptBreakdown.map(({ dept, total, attended }) => (
                      <button key={dept} onClick={() => setDeptFilter(dept === deptFilter ? '' : dept)} style={{ padding: '5px 12px', borderRadius: '20px', border: 'none', fontSize: '12px', fontWeight: '600', cursor: 'pointer', fontFamily: 'Inter, sans-serif', background: deptFilter === dept ? '#051c2c' : '#f2f4f6', color: deptFilter === dept ? '#ffffff' : '#5a6878' }}>
              ...
              <div style={{ marginTop: '8px' }}>
                <div style={styles.sectionLabel}>
                  ...
                  <span style={{ fontSize: '11px', color: '#9baabb', fontWeight: '400', marginLeft: '8px' }}>
                    {filteredEnrolled.length} {deptFilter ? `from ${deptFilter}` : 'total'} · {filteredEnrolled.filter(l => l.attended).length} attended
                  </span>
                </div>
                {enrollLoading ? (
                  <div style={{ padding: '24px', textAlign: 'center', color: '#9baabb', fontSize: '13px' }}>Loading learners...</div>
                        ...
                        {filteredEnrolled.map(learner => (
                          <tr key={learner.id} style={{ borderBottom: '1px solid #f0f2f4' }}>
                            <td style={{ padding: '10px 12px' }}>
                              <button style={{ background: 'none', border: 'none', cursor: 'pointer', fontSize: '13px', fontWeight: '600', color: '#051c2c', padding: 0, fontFamily: 'Inter, sans-serif', display: 'flex', alignItems: 'center', gap: '8px' }} onClick={() => setProfileLearner(learner)}>
                                <div style={{ width: '26px', height: '26px', borderRadius: '50%', background: '#051c2c', color: '#ffffff', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '9px', fontWeight: '700', flexShrink: 0 }}>
                            ...
                            <td style={{ padding: '10px 12px' }}>
                              {learner.attended
                                ? <span style={{ background: learner.feedback_submitted ? '#dcfce7' : '#fef9c3', color: learner.feedback_submitted ? '#15803d' : '#a16207', padding: '2px 8px', borderRadius: '10px', fontSize: '11px', fontWeight: '600' }}>
                            ...
                            <td style={{ padding: '10px 12px', fontSize: '12px', color: '#9baabb', whiteSpace: 'nowrap' }}>
                              {learner.enrolled_at ? new Date(learner.enrolled_at).toLocaleDateString('en-GB') : '—'}
                            </td>
                          </tr>
                        ))}
                      </tbody>
                  ...
                  <div style={{ padding: '20px', background: '#f8f9fa', borderRadius: '8px', border: '1px solid #e8ecf0', textAlign: 'center', fontSize: '13px', color: '#9baabb' }}>
                    {deptFilter ? `No learners from ${deptFilter} enrolled in this course.` : 'No learners enrolled in this course yet.'}
                  </div>
                )}
              </div>
            </div>
            ...
            <div style={styles.modalBody}>
              {trainerPopup.notFound ? (
                ...
                <>
                  ...
                  <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '14px' }}>
                    {[['Phone', trainerPopup.phone || '—'], ['Email', trainerPopup.email || '—']].map(([k, v]) => (
                      <div key={k}>
                        <div style={{ fontSize: '10px', fontWeight: '700', color: '#9baabb', textTransform: 'uppercase', letterSpacing: '0.5px', marginBottom: '4px' }}>{k}</div>
                        <div style={{ fontSize: '13px', fontWeight: '500', color: '#051c2c' }}>{v}</div>
                      </div>
      ...
      {feedbackPopup && (
        <div style={{ ...styles.overlay, zIndex: 1100 }} onClick={() => setFeedbackPopup(null)}>
          <div style={{ ...styles.modal, maxWidth: '620px' }} onClick={e => e.stopPropagation()}>
            ...
            <div style={styles.modalBody}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '14px 16px', background: '#f8f9fa', borderRadius: '10px', border: '1px solid #e8ecf0', marginBottom: '18px' }}>
                <div>
                ...
                {!isHod && (
                  <div style={styles.periodToggleSmall}>
                    ...
                    <button onClick={() => handleToggleManual(true)}  style={{ ...styles.periodBtnSmall, ...(manualOverride  ? styles.periodBtnActiveSmall : {}) }}>Manual</button>
                  </div>
                )}
              </div>
                ...
                <div style={{ marginBottom: '18px', padding: '14px 16px', background: '#fef9c3', borderRadius: '10px', border: '1px solid #fde68a' }}>
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
                    <div style={{ display: 'flex', gap: '4px' }}>
                      {[1,2,3,4,5].map(star => (
                        <button key={star} style={{ background: 'none', border: 'none', cursor: 'pointer', fontSize: '26px', padding: '2px', color: star <= manualValue ? '#c8973a' : '#d4d9dd' }} onClick={() => setManualValue(star)}>★</button>
                      ))}
                    </div>
                    <span style={{ fontSize: '14px', fontWeight: '700', color: '#c8973a' }}>{Math.round((manualValue / 5) * 100)}%</span>
                    <button onClick={handleSaveManualValue} style={{ marginLeft: 'auto', background: '#051c2c', color: '#ffffff', border: 'none', borderRadius: '6px', padding: '6px 14px', fontSize: '12px', fontWeight: '600', cursor: 'pointer', fontFamily: 'Inter, sans-serif' }}>Save</button>
                  </div>
                </div>
              )}
              ...
              {feedbackLoading ? (
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: '8px', maxHeight: '300px', overflowY: 'auto' }}>
                  {feedbackList.map((f, i) => (
                    <div key={i} style={{ padding: '12px 14px', borderRadius: '8px', border: '1px solid #e8ecf0', background: '#f8f9fa' }}>
                      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '6px' }}>
                        <span style={{ fontSize: '13px', fontWeight: '600', color: '#051c2c' }}>{f.learner_name}</span>
                        <span style={{ fontSize: '11px', color: '#9baabb' }}>{f.submitted_at ? new Date(f.submitted_at).toLocaleDateString('en-GB') : '—'}</span>
                      </div>
                      <div style={{ fontSize: '11px', color: '#5a6878', marginBottom: '6px' }}>{f.department_name || '—'} · {f.emp_id || '—'}</div>
                      ...
                      {f.testimonial && (
                        <div style={{ fontSize: '12px', color: '#5a6878', marginTop: '8px', fontStyle: 'italic', borderTop: '1px solid #e8ecf0', paddingTop: '8px' }}>"{f.testimonial}"</div>
                      )}
                    </div>
                  ))}
                </div>
  ...
  return (
    <>
      ...
      <div style={{ marginBottom: '14px' }}>
        <label style={{ fontSize: '11px', fontWeight: '700', color: '#5a6878', textTransform: 'uppercase', letterSpacing: '0.5px', display: 'block', marginBottom: '5px' }}>Description</label>
        <textarea value={f.description} onChange={e => setF({...f, description: e.target.value})} placeholder="What will participants learn?"
      ...
      <div style={{ marginBottom: '14px' }}>
        <F label="Training Type *" value={f.trainingType} onChange={v => setF({...f, trainingType: v})} type="select" options={['Developmental', 'Mandatory']} />
      </div>
      <div style={styles.formGrid}>
        ...
        <F label="Trainer"               value={f.trainer}        onChange={v => setF({...f, trainer: v})}
          type="select" options={trainers.length > 0 ? trainers.map(t => ({ label: t.name, value: t.name })) : [{ label: 'No trainers yet', value: '' }]} />
          ...
          <div style={{ display: 'flex', gap: '4px' }}>
            {[1,2,3,4,5].map(star => (
              <button key={star} type="button"
                style={{ background: 'none', border: 'none', cursor: 'pointer', fontSize: '30px', padding: '2px', lineHeight: 1, color: star <= (hoverStar || +f.stars || 0) ? '#c8973a' : '#d4d9dd', transition: 'color 0.1s' }}
                onMouseEnter={() => setHoverStar(star)} onMouseLeave={() => setHoverStar(0)}
                onClick={() => setF({...f, stars: star})}>★</button>
            ))}
          </div>
          {+f.stars > 0 && <span style={{ fontSize: '15px', fontWeight: '700', color: '#c8973a' }}>{Math.round((+f.stars / 5) * 100)}%</span>}
          {+f.stars > 0 && <button type="button" style={{ background: 'none', border: 'none', cursor: 'pointer', fontSize: '11px', color: '#9baabb', textDecoration: 'underline', fontFamily: 'Inter, sans-serif' }} onClick={() => setF({...f, stars: 0})}>Clear</button>}
        </div>
      </div>
    </>
  );
